...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, 
                ...
                (SELECT content FROM " . DB_PREFIX . "product_gift pg WHERE pg.product_id = p.product_id AND pg.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((pg.date_start = '0000-00-00' OR pg.date_start < NOW()) 
                AND (pg.date_end = '0000-00-00' OR pg.date_end > NOW())) ORDER BY pg.priority ASC, pg.price ASC LIMIT 1) AS gift, 
                ...
                (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 
	...
	public function getProducts($data = array()) {
		...
		(SELECT price FROM " . DB_PREFIX . "product_discount pd2 
		 ...
		 (SELECT description FROM " . DB_PREFIX . "product_gift pg 
		 WHERE pg.product_id = p.product_id 
		 AND pg.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' 
		 AND ((pg.date_start = '0000-00-00' OR pg.date_start < NOW()) 
		 AND (pg.date_end = '0000-00-00' OR pg.date_end > NOW())) 
		 ORDER BY pg.priority ASC LIMIT 1) AS gift";
	...
	public function getBestSellerProducts($limit = 0) {
		...
		if (!$product_data) {
			...
			$sql = "SELECT op.product_id, SUM(op.quantity) AS total FROM " . DB_PREFIX . "order_product op 
                ...
                GROUP BY op.product_id ORDER BY total DESC ";
    ...
    public function getNewestProducts($limit = 0) {
		$product_data = $this->cache->get('product.newest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id') . '.' . (int)$limit);
		...
		if (!$product_data) {
            ...
            LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) 
            WHERE p.status = '1' AND p.date_available <= NOW() 
            AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND p.is_receive = 1 
			...
			$this->cache->set('product.newest.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id') . '.' . (int)$limit, $product_data);
		}
	...
	public function getProductGift($product_id) {
	    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_gift pg
                                   WHERE pg.product_id = " . (int)$product_id . "
                                        AND pg.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'
                                        AND ((pg.date_start = '0000-00-00' OR pg.date_start < NOW())
                                             AND (pg.date_end = '0000-00-00' OR pg.date_end > NOW()))
                                   ORDER BY pg.priority ASC LIMIT 1");
